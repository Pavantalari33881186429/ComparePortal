pipeline{
  ...
  tools{
   ...
   environment {
        DOCKER_IMAGE = 'vayuputra123/compareportal'
  ...
  stages{
    ...
    stage('Get code from Git'){
       steps{
           ...
           git branch: 'main', url: 'https://github.com/Pavantalari33881186429/ComparePortal.git'
       }
    }
    ...
    stage('CODE ANALYSIS with SONARQUBE') {
          ...
          steps {
            withSonarQubeEnv('sonar') {
               sh '''${scannerHome}/bin/sonar-scanner -Dsonar.projectKey=ComparePortal \
                   -Dsonar.projectName=ComparePortal \
